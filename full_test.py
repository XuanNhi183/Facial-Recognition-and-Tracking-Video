...
def train_classifier():
    data_dir = "D:\\CPV303\\Face_detection\\data\\"
...
def start_video():
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, clf, mycursor):
        ...
        for (x, y, w, h) in features:
            ...
            mycursor.execute("select name from dang_ki where id=" + str(id))
            s = mycursor.fetchone()

            if s and confidence > 70:
                s = ''.join(s)
                cv2.rectangle(img, (x, y), (x + w, y + h), (0, 255, 0), 2)
                cv2.putText(img, s, (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 1, cv2.LINE_AA)
            else:
            ...
            for orientation, message, count in orientations:
                ...
                while captured < count:
                    ...
                    if cropped_face is not None:
                        ...
                        file_name_path = f"data/user.{id}.{img_id}.{orientation}.jpg"
